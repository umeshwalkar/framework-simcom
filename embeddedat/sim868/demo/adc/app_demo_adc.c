...
eat_bool eat_module_test_adc(u8 param1, u8 param2)
{
    ...
    else if( 8== param1)
    {
        if( 1 == param2 )
        {
            eat_adc_get_ms(EAT_ADC0, 500, adc_cb_proc);
            eat_trace("ADC test 8,1 start");
        }else
        {
            eat_adc_get_ms(EAT_ADC0, 0, NULL);
            eat_trace("ADC test 8,x stop");
        }
    }
