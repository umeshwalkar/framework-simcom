...
void app_main(void *data)
{
	...
	app_mem_init();
	eat_modem_set_poweron_urc_dir(EAT_USER_1);		// set user1 to receive the messages from modem	
    ...
    while(EAT_TRUE)
    {
        ...
        switch(event.event)
        {
            case EAT_EVENT_MDM_READY_RD:
            {
				...
				u16 len = eat_modem_read(buf,2048);
				if(len > 0)
				{	
					buf[len] = '\0';
					eat_trace("app_main ###### ReFrModem:%s,len = %d",buf,len);
				}
				break;
            }
